...
import { generarPagoServiciosPDF, agruparPorDia, imagenComoDataUrl } from '../utils/pagoServiciosPdf'
...
export default function Historial() {
  ...
  // servicios, agrupado por día. Descarga el PDF Y lo deja guardado como
  // "pendiente de firma" para que la manicurista lo firme desde su cuenta ----
  const { user } = useAuth()
  const [generandoPago, setGenerandoPago] = useState(false)
  const [avisoPago, setAvisoPago] = useState('')
  const [avisoPagoOk, setAvisoPagoOk] = useState(false)

  const generarPagoManicurista = async () => {
    if (!filtroManicurista) {
      setAvisoPagoOk(false)
    ...
    doc.save(`pago-servicios_${manicuristaNombre.replace(/\s+/g, '-').toLowerCase()}_${desde}_a_${hasta}.pdf`)

    const { error } = await supabase.from('pagos_manicuristas').insert({
      manicurista_id: filtroManicurista,
      fecha_desde: desde,
      fecha_hasta: hasta,
      detalle: filasPorDia,
      total,
    ...
    if (error) {
      setAvisoPagoOk(false)
      setAvisoPago('El PDF se descargó, pero no se pudo enviar a firmar: ' + error.message)
    } else {
      setAvisoPagoOk(true)
  ...
  return (
    <div>
      ...
      {avisoPago && (
        <p className="text-xs mb-3" style={{ color: avisoPagoOk ? 'var(--color-success)' : 'var(--color-danger)' }}>{avisoPago}</p>
      )}
      <div className="mb-4" />
